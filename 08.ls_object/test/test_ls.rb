...
describe List::Command do
  ...
  describe 'オプション -l ありのls' do
    before do
      ...
      @last_update_time = Time.now.strftime('%_m %_d %H:%M %Y')
    ...
    describe 'ディレクトリ・ファイル指定なし' do
      it 'ファイルとディレクトリの一覧が最大1列で表示されること' do
        stdout = <<~STDOUT
          -r-xrwxrwx 1 maeda-m maeda-m    0 11 22 03:04 2111 006-ﾆﾎﾝｺﾞ.txt
          drwxr-xr-x 3 maeda-m maeda-m 4096  2 22 22:22 2122 007-日本語のディレクトリ
          -rw------- 1 maeda-m maeda-m   10 12 31 23:59 2000 008.txt
          -rwxr-xr-- 1 maeda-m maeda-m    3  9  9 09:09 2199 009.txt
          --wx-w---x 1 maeda-m maeda-m    5  8  8 08:08 2188 010.txt
          ---------- 1 maeda-m maeda-m   12  7  7 07:07 2177 011.txt
    ...
    describe 'オプション -a を追加した場合' do
      it 'ファイル（隠しファイルあり）とディレクトリの一覧が最大1列で表示されること' do
        ...
        assert_match(/\Adrwx------\s+\d+ maeda-m maeda-m\s+\d+\s+\d{1,2}\s+\d{1,2}\s+\d{2,2}:\d{2,2} \d{4,4} \.\z/, lines[0])
        assert_match(/\Adrwxrwxrwx\s+\d+ root    root   \s+\d+\s+\d{1,2}\s+\d{1,2}\s+\d{2,2}:\d{2,2} \d{4,4} \.\.\z/, lines[1])
        assert_match(/\A-rw-r--r--\s+1 maeda-m maeda-m\s+0  1  1 01:01 1999 \.ignore\z/, lines[2])
        assert_match(/\A-r-xrwxrwx\s+1 maeda-m maeda-m\s+0 11 22 03:04 2111 006-ﾆﾎﾝｺﾞ\.txt\z/, lines[3])
    ...
    describe 'オプション -ar を追加した場合' do
      it 'ファイル（隠しファイルあり）とディレクトリの一覧が最大1列で表示されること' do
        ...
        assert_match(/\A-r-xrwxrwx\s+1 maeda-m maeda-m\s+0 11 22 03:04 2111 006-ﾆﾎﾝｺﾞ\.txt\z/, lines[6])
        assert_match(/\A-rw-r--r--\s+1 maeda-m maeda-m\s+0  1  1 01:01 1999 \.ignore\z/, lines[7])
        assert_match(/\Adrwxrwxrwx\s+\d+ root    root   \s+\d+\s+\d{1,2}\s+\d{1,2}\s+\d{2,2}:\d{2,2} \d{4,4} \.\.\z/, lines[8])
        assert_match(/\Adrwx------\s+\d+ maeda-m maeda-m\s+\d+\s+\d{1,2}\s+\d{1,2}\s+\d{2,2}:\d{2,2} \d{4,4} \.\z/, lines[9])
      end
    end
  end
end
